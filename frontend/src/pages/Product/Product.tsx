      ...
      <div className="mt-10 max-w-xl mx-auto flex-col space-x-0 space-y-3 md:flex md:max-w-none md:mx-0 md:flex-row md:space-y-0 md:items-start md:space-x-4">
        <div className="lg:flex-1 lg:flex items-start space-y-3 lg:space-y-0 lg:space-x-4">
          <div className="w-full">
            {props.productDetails?.image.length > 0 && (
              <div
                className="w-full relative group shadow-lg rounded-md overflow-hidden"
            ...
            {isOpen && (
              <Lightbox
                mainSrc={props.productDetails?.image[photoIndex]}
                nextSrc={
                  props.productDetails?.image[
                    (photoIndex + 1) % props.productDetails?.image.length
                  ]
                }
                prevSrc={
                  props.productDetails?.image[
                ...
                onMovePrevRequest={() =>
                  setPhotoIndex(
                    (prev) =>
                      (prev + props.productDetails?.image.length - 1) %
                      props.productDetails?.image.length
                  )
                }
                onMoveNextRequest={() =>
                  setPhotoIndex(
            ...
            {/* <img
              src={props.productDetails?.image[0]}
