  ...
  return (
    <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 relative pt-8 pb-10">
      <div className="mx-auto w-full">
        <Carousel {...settings} enableAutoPlay autoPlaySpeed={1800}>
          {props.topProducts.map((product, idx) => (
            <div
              key={idx}
              className="px-3 md:px-0 shadow-lg rounded-lg overflow-hidden relative items-center justify-center py-20 sm:py-28 w-full inset-0"
            >
              <Link to={`/product/${product._id}`}>
                ...
                <div className="mx-auto max-w-sm sm:max-w-xl text-center relative z-10">
                  <h2 className="group-hover:underline text-4xl tracking-tight leading-10 font-extrabold text-white sm:text-5xl sm:leading-none md:text-6xl">
                    {product?.name}
                  </h2>
                  <p className="mt-2 text-gray-300 text-sm line-clamp-2 tracking-wide leading-6">
                    {product?.description}
                  </p>
                </div>
